...
import { placeRFQOrder } from "./api/order";
...
export const createMarketOrder = async ({
  ...
  const order = Order.create({
    ...
    price: roundToMinChange(
      side === "BUY"
        ? market.marketStats.markPrice.times(Decimal(1).plus(SLIPPAGE))
        : market.marketStats.markPrice.times(Decimal(1).minus(SLIPPAGE)),
  ...
  const result = await placeRFQOrder({ order });
